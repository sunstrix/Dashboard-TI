...
2. Abra o `.env` e substitua o valor de `GOOGLE_DRIVE_API_KEY` pela sua chave real (veja como gerar abaixo)
...
## 📂 Estrutura do Projeto
    ...
    ├── drive_client.py             # Integração com Google Drive API v3
...
Os arquivos são nomeados no padrão: `CPFANI_Hardware_Snapshot_<ID>.txt`
...
> ⚠️ O **Nome_Computador** e o **nome do arquivo** NÃO são usados para deduplicação, pois podem mudar. Apenas o Hardware ID é confiável.
...
**Solução**:
- Verifique o `DRIVE_FOLDER_ID` no `config.py`
